function applySnippet(configData) {
    if (configData.bpxSnippet && (window.location.href.indexOf("core") > -1) && $('#bpx-snippet-block').length == 0) {
        var html = '<div id="bpx-snippet-block" style="display:flex; margin-right:40px">' +
            ...
            '</div>' +
            '<div class="bpx-snippet-note" style="color:white;font-size:11px;">click and paste<br> with ctrl+v</div>' +
            '<style>#bpx-snippet-block button {cursor: pointer; margin-right:5px; min-width:30px; height:25px; font-size:11px; background-color:white; border-radius: 4px; border:0px ;box-shadow: 1px 1px 2px rgba(0, 0, 0, 0.5)} #bpx-snippet-block button:focus {cursor: pointer; background-color:grey;} .bpx-number-snippet{margin-right:10px;} .ap-gn__area--last{display:contents;}</style>' +
            '</div>';
